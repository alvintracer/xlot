...
// [Phase 2: WalletConnect v2 연동] 수정 내역:
// 1. WEB3_WALLETS에 WALLETCONNECT 항목 추가
// 2. handleConnectWallet()에 WalletConnect 케이스 추가
//    - thirdweb createWallet("walletConnect") 활용
//    - useConnect hook으로 연결 → 주소 추출
// 3. WalletSlot wallet_type 'WALLETCONNECT' 추가
// 4. WalletConnect 연결 시 label에 원래 지갑 이름 반영
// 기존 코드는 100% 유지

import { useState, useEffect } from "react";
import { X, Loader2, Edit2, AlertTriangle, KeyRound, CheckCircle2, Copy, ExternalLink, ShieldCheck, ArrowRight, Smartphone, QrCode } from "lucide-react";
...
const WEB3_WALLETS = [
  ...
  // ✨ [Phase 2 추가] WalletConnect — MPC 지갑 포함 모든 WC v2 지갑
  { id: 'WALLETCONNECT',  name: 'WalletConnect',    icon: '🔗', bg: 'bg-cyan-500/10',   color: 'text-cyan-400',    supported: ['EVM'] },
];
...
export function AddWalletModal({ onClose, onSuccess }: Props) {
  ...
  // ✨ [Phase 2] thirdweb useConnect hook
  ...
  // ✨ [Phase 2] WalletConnect 상태
  const [wcConnecting, setWcConnecting] = useState(false);
  const [wcConnectedWalletName, setWcConnectedWalletName] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchServerIp = async () => {
      try {
        ...
        setCurrentIp(data.ip || "49.247.139.241");
      } catch (e) {
        setCurrentIp("49.247.139.241");
      }
    };
    fetchServerIp();
  }, []);
  ...
  // ✨ [Phase 2] WalletConnect 전용 핸들러
  // thirdweb의 walletConnect()를 사용 — QR 코드 팝업은 thirdweb SDK가 자동 처리
  const handleWalletConnect = async () => {
    setWcConnecting(true);
    setAddress("");
    setWcConnectedWalletName(null);

    try {
      const connectedWallet = await connect(async () => {
        const wc = createWallet("walletConnect");

        // QR 코드 또는 딥링크 연결 시도
        await wc.connect({ 
          client,
          // walletConnect configuration properties
          projectId: import.meta.env.VITE_WALLETCONNECT_PROJECT_ID || "",
          appMetadata: {
            name: "xLOT",
            description: "Compliant Wallet Aggregator",
      ...
      // 연결된 지갑에서 주소 추출
      if (!connectedWallet) throw new Error("지갑 연결에 실패했습니다. (connectedWallet is null)");
      
      const connectedAccount = connectedWallet.getAccount();
      if (!connectedAccount?.address) throw new Error("주소를 가져오지 못했습니다.");

      // 연결된 지갑 이름 (Fireblocks, Upbit MPC 등)
      const wcWallet = connectedWallet as any;
      const walletName = wcWallet?.session?.peer?.metadata?.name || "WalletConnect 지갑";

      setAddress(connectedAccount.address);
      setDetectedType('WALLETCONNECT');
      setWcConnectedWalletName(walletName);
      setLabel(`${walletName}`);

    } catch (e: any) {
      console.error("[WalletConnect]", e);
      // 사용자가 직접 닫은 경우는 에러 알림 생략
      if (!e.message?.includes("User rejected") && !e.message?.includes("closed")) {
        alert("WalletConnect 연결 실패: " + (e.message || "알 수 없는 오류"));
      }
    } finally {
  ...
  const handleAdd = async () => {
    ...
    try {
      if (mainTab === "WEB3") {
        ...
        // WalletConnect는 항상 EVM 주소
        const walletType = detectedType || "MANUAL";
        if (targetChain === 'EVM' || walletType === 'WALLETCONNECT') {
          await addWeb3Wallet(activeAccount.address, address, label || "EVM Wallet", walletType);
        } else if (targetChain === 'SOL') {
          await addSolanaWallet(activeAccount.address, address, label || "Solana Wallet");
        } else if (targetChain === 'TRON') {
          await addTronWallet(activeAccount.address, address, label || "Tron Wallet");
        } else if (targetChain === 'BTC') {
          await addBitcoinWallet(activeAccount.address, address, label || "Bitcoin Wallet");
        }
      } else if (mainTab === "CEX") {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      saveImportedKey(targetChain, validationResult.address, validationResult.formattedKey, passcode);

      const walletLabel = label || `${targetChain} Import Wallet`;
      if (targetChain === 'EVM') await addWeb3Wallet(activeAccount.address, validationResult.address, walletLabel, "MANUAL");
      else if (targetChain === 'SOL') await addSolanaWallet(activeAccount.address, validationResult.address, walletLabel);
      else if (targetChain === 'TRON') await addTronWallet(activeAccount.address, validationResult.address, walletLabel);
      else if (targetChain === 'BTC') await addBitcoinWallet(activeAccount.address, validationResult.address, walletLabel);
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2 overflow-x-auto scrollbar-hide">
                {CHAINS.map(c => (
                  <button key={c.id} onClick={() => { setTargetChain(c.id as any); setAddress(""); }}
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all whitespace-nowrap flex-1
                      ${targetChain === c.id
                        ? `bg-${c.color}-500/20 border-${c.color}-500/50 text-${c.color}-400`
            ...
            {web3Mode === "CONNECT" ? (
              <div className="space-y-3 pt-2">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {WEB3_WALLETS.map((wallet) => {
                    ...
                    const isWCLoading = wallet.id === 'WALLETCONNECT' && (wcConnecting || isConnecting);

                    return (
                      <button
                        key={wallet.id}
                        onClick={() => handleConnectWallet(wallet.id)}
                        disabled={loading || isWCLoading || !isSupported}
                        className={`flex flex-col items-center justify-center p-4 rounded-2xl border transition-all gap-2 relative overflow-hidden
                          ${detectedType === wallet.id && address
                            ? 'ring-2 ring-cyan-500 bg-slate-800 border-transparent'
                            : 'bg-slate-900 border-slate-800'}
                          ${!isSupported ? 'opacity-30 cursor-not-allowed grayscale' : 'hover:border-slate-600 hover:bg-slate-800'}
                          ${wallet.id === 'WALLETCONNECT' ? 'col-span-2 flex-row gap-3 py-3' : ''}`}
                      >
                        {isWCLoading ? (
                          <Loader2 className="animate-spin text-cyan-400" size={20} />
                        ) : (
                          <div className={`${wallet.id === 'WALLETCONNECT' ? 'w-8 h-8' : 'w-10 h-10'} rounded-full flex items-center justify-center text-xl ${wallet.bg} ${wallet.color}`}>
                            {wallet.id === 'WALLETCONNECT' ? <QrCode size={16} className="text-cyan-400" /> : wallet.icon}
                          </div>
                        )}
                        <div className={wallet.id === 'WALLETCONNECT' ? 'text-left' : ''}>
                          <span className="text-xs font-bold text-slate-300 block">{wallet.name}</span>
                          {wallet.id === 'WALLETCONNECT' && (
                            <span className="text-[10px] text-slate-500">Fireblocks, 업비트 MPC 등 모든 WC 지갑</span>
                          )}
                        </div>
                      </button>
                    );
                ...
                {/* ✨ [Phase 2] WalletConnect 연결 성공 시 표시 */}
                {detectedType === 'WALLETCONNECT' && address && wcConnectedWalletName && (
                  <div className="mt-2 animate-fade-in bg-cyan-500/10 p-4 rounded-xl border border-cyan-500/30">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-bold text-cyan-400 flex items-center gap-1">
                        <CheckCircle2 size={12} /> WalletConnect 연결됨
                      </span>
                      <span className="text-[10px] text-slate-400 bg-slate-800 px-2 py-0.5 rounded">{wcConnectedWalletName}</span>
                    </div>
                    <p className="text-[10px] text-slate-500 font-mono break-all">{address}</p>
                  </div>
                )}

                {/* 기존 지갑 연결 감지 표시 (WC 제외) */}
                {address && detectedType !== 'WALLETCONNECT' && (
                  <div className="mt-4 animate-fade-in bg-slate-950 p-4 rounded-xl border border-cyan-500/30">
                    ...
                    <input value={address} onChange={(e) => setAddress(e.target.value)}
                      className="w-full bg-slate-900 text-white p-3 rounded-lg outline-none border border-slate-800 focus:border-cyan-400 text-xs font-mono" />
                  </div>
                )}
              </div>
            ) : (
              <div className="space-y-3 animate-fade-in pt-2">
                <input value={address} onChange={(e) => setAddress(e.target.value)}
                  placeholder={`${targetChain} 주소 직접 입력`}
                  className="w-full bg-slate-950 text-white p-4 rounded-2xl outline-none border border-slate-800 focus:border-cyan-500 text-sm font-mono" />
              </div>
            )}
            ...
            {address && (
              <div className="space-y-4 animate-fade-in-up pt-4 border-t border-slate-800 mt-4">
                <input value={label} onChange={(e) => setLabel(e.target.value)}
                  placeholder="지갑 별칭 (선택)"
                  className="w-full bg-slate-950 text-white p-4 rounded-2xl outline-none border border-slate-800 focus:border-cyan-500 text-sm" />
                <button onClick={handleAdd} disabled={loading}
                  className="w-full py-4 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-2xl font-bold text-white shadow-lg">
        ...
        {mainTab === "CEX" && (
          <div className="space-y-4 animate-fade-in">
            <div className="grid grid-cols-4 gap-2 mb-4">
              {CEX_LIST.map(cex => (
                <button key={cex.id} onClick={() => setSelectedCex(cex.id)}
            ...
            <div className="bg-slate-950 rounded-2xl border border-slate-800 p-4 space-y-4">
              <div>
                ...
                <div className="flex items-center gap-2">
                  <code className="flex-1 bg-slate-900 p-2.5 rounded-lg border border-slate-800 text-indigo-300 font-mono text-xs text-center">{currentIp}</code>
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-500 mt-2 text-center">* '자산 조회' 권한만 체크해주세요.</p>
              </div>
            </div>
            ...
            <div className="pt-2 border-t border-slate-800">
              <div className="space-y-3">
                <div className="relative">
                  ...
                  <input value={accessKey} onChange={(e) => setAccessKey(e.target.value)} type="text" placeholder="Access Key"
                    className="w-full bg-slate-950 text-white p-3 pl-9 rounded-xl outline-none border border-slate-800 focus:border-indigo-500 text-xs font-mono" />
                </div>
                <div className="relative">
                  <KeyRound size={14} className="absolute left-3 top-4 text-slate-500" />
                  <input value={secretKey} onChange={(e) => setSecretKey(e.target.value)} type="password" placeholder="Secret Key"
                    className="w-full bg-slate-950 text-white p-3 pl-9 rounded-xl outline-none border border-slate-800 focus:border-indigo-500 text-xs font-mono" />
                </div>
              </div>
            </div>
            ...
            <button onClick={handleAdd} disabled={loading}
              className="w-full py-3.5 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl font-bold text-white shadow-lg text-sm flex items-center justify-center gap-2 hover:opacity-90 transition-opacity">
        ...
        {mainTab === "IMPORT" && (
          <div className="space-y-4 animate-fade-in">
              ...
              <div className="flex gap-2 overflow-x-auto scrollbar-hide">
                {CHAINS.map(c => (
                  <button key={c.id} onClick={() => setTargetChain(c.id as any)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all whitespace-nowrap flex-1
                      ${targetChain === c.id ? `bg-${c.color}-500/20 border-${c.color}-500/50 text-${c.color}-400` : 'bg-slate-950 border-slate-800 text-slate-500 hover:border-slate-700'}`}>
                    {c.label}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <input value={label} onChange={(e) => setLabel(e.target.value)}
                placeholder={`${targetChain} 가져온 지갑`}
            ...
            <button onClick={handleImport} disabled={loading}
              className="w-full py-4 bg-gradient-to-r from-emerald-500 to-teal-500 rounded-2xl font-bold text-white shadow-lg mt-2 flex items-center justify-center gap-2">
